...
<div class="px-5 py-5 w-2/4">
    <form id="jadwalEditForm" method="post" class="space-y-6" action="{{route('ruang.update', $ruang->id)}}">
    @csrf
    @method('patch')
        <div>
            <label for="kode_ruang" class="flex-grow block font-medium text-sm text-gray-700">Kode Ruang</label>
            <input name="kode_ruang" value="{{old('kode_ruang') != null ? old('kode_ruang') : $ruang->kode_ruang }}" id="kode_ruang" type="text" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Masukan kode ruang..">
